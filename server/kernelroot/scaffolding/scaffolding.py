...
from kernelroot.core.config import BASE_DIR
...
def update_scheduler_registry(name: str, dotted: str, scheduler_map: dict):
    reg_path = os.path.join(BASE_DIR, "kernelroot", "scheduler_registry.py")
    ...
    with open(reg_path, "w") as f:
        f.write(src)
    import kernelroot.scheduler_registry as _sr
    importlib.reload(_sr)
    scheduler_map[name] = dotted


def update_router_registry(name: str, mod_path: str):
    reg_path = os.path.join(BASE_DIR, "kernelroot", "router_registry.py")
    with open(reg_path) as f:
        src = f.read()
...
def unregister_scheduler(name: str, scheduler_map: dict):
    ...
    # router_registry.py
    rr_path = os.path.join(BASE_DIR, "kernelroot", "router_registry.py")
...
\"\"\"
{Name}Scheduler — generated by the PMK scheduler assistant.
...
Generated by the PMK scheduler assistant.
